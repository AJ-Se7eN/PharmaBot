...
def welcome(message):
    msg = bot.send_sticker(
        message.chat.id,
        "CAACAgIAAxkBAAI3eV56Cj5ob-7NfqeTZ4HBdwvTZXhbAAJlAgADOKAKtGgxreR2qoYYBA",
    )
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row("👩🏻‍⚕️ Продавец-фармацевт", "🧴 Покупатель")
    bot.send_message(
        message.chat.id,
        "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот созданный чтобы помочь, Вам!".format(
...
def pharmac_or_buyer(message):
    if message.text == "🧴 Покупатель" or message.text == "🚪 Назад":
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row(
            "👩🏻‍⚕️ Аптеки в Районах Бишкека",
            "🧴 Поиск Маски по названию и адресу",
            "🚪 Выйти",
        )
        msg = bot.send_message(
            message.chat.id, "Выберите пожалуйста:", reply_markup=markup
        )
        bot.register_next_step_handler(msg, process_search_pharm)  # 119 строка
    elif message.text == "👩🏻‍⚕️ Продавец-фармацевт":
        msg = bot.reply_to(
            message, "Название вашей аптеки?", reply_markup=types.ReplyKeyboardRemove()
        )
...
def process_apteka_region(message):
    try:
        ...
        msg = bot.send_message(
            message.chat.id,
            f"Название вашей аптеки: {apteka.name} \n Адрес вашей аптеки: \
        {apteka.adress} \n Ваш район: {apteka.region} \nАптека успешно создана!",
            reply_markup=markup,
        )
...
def yes_no_mask(message):
    if (
        message.text == "Нет"
    ):  # если ответ пользователя 'нет', то constanta принимает False
        ...
        message.text == "Да"
    ):  # если ответ пользователя 'да', то constanta принимает True
...
def process_register_pharm(message):
    try:
        ...
        msg = bot.send_message(
            message.chat.id,
            "Можете нажать на кнопку и начнется чудо😄",
            reply_markup=markup,
        )
...
def process_search_pharm(message):
    if message.text == "👩🏻‍⚕️ Аптеки в Районах Бишкека":
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row(
            "Ленинский", "Свердловский", "Первомайский", "Октябрьский", "🚪 Назад"
        )
        ...
        msg = bot.send_message(
            message.chat.id,
            "Введите пожалуйста название аптеки:",
...
def process_search_names(message):
    ...
    Apteka.search_name = (
        search_name  # записываю в переменную для дальнейшего сравнения 156 строка
    )
...
def process_search_adress(message):
    try:
        ...
        if x == 0:
            bot.send_message(
                message.chat.id, "Аптека с такими данными не зарегистрированна!"
            )
        # кнопка назад
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row("🚪 Назад")
        send = bot.send_message(
            message.chat.id, "Желаете вернуться назад?", reply_markup=markup
        )
...
def exit_(message):
    ...
    markup.add("/start")
    msg = bot.send_message(
        message.chat.id, "Можете нажать на кнопку и начнется чудо😄", reply_markup=markup
    )
    bot.register_next_step_handler(msg, welcome)
